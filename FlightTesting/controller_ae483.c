                     ...
                     const uint32_t tick)
{
  if (RATE_DO_EXECUTE(ATTITUDE_RATE, tick)) {
    ...
    if (setpoint->mode.z == modeDisable) {
      ...
      // tau_y = -0.00223607f * (o_x - o_x_des) -0.00654857f * theta -0.00194559f * v_x -0.00108695f * w_y - 0.001f*alpha;
      // tau_z = -0.00100000f * psi -0.00102777f * w_z;
      // f_z = -0.21447611f * (o_z - o_z_des) -0.19618318f * v_z + 0.43f;

      tau_x = 0.00264575f * (o_y - o_y_des) -0.00666549f * phi + 0.00209651f * v_y -0.00109968f * w_x;
      tau_y = 0.005f *(0.00000716f * (o_x - o_x_des)  + 0.00234414f * theta + 0.00024953f * v_x + 0.00028499f * w_y + 0.00009f * alpha);
      tau_z = -0.00100000f * psi -0.00102029f * w_z;
      f_z = -0.70710678f * (o_z - o_z_des) -0.40277214f * v_z + 0.47464000f;


      // tau_x = 0.00264575f * (o_y - o_y_des) -0.00666549f * phi + 0.00209651f * v_y -0.00109968f * w_x;
      // tau_y = (0.00001000f * (o_x - o_x_des) +0.00145201f * theta + 0.00027099f * v_x +0.00021558f * w_y + 0.00011213f * alpha);
      // tau_z = -0.00100000f * psi -0.00102029f * w_z;
      // f_z = -0.70710678f * (o_z - o_z_des) -0.40277214f * v_z + 0.43164000f;

      // tau_x = 0.00264575f * (o_y - o_y_des) -0.00671612f * phi + 0.00210301f * v_y -0.00111592f * w_x;
      // tau_y = 0.00100000f * (o_x - o_x_des) -0.02618588f * theta + 0.02656604f * v_x -0.00197885f * w_y + 0.01046226f * alpha + 0.00701502f * alpha_dot;
      // tau_z = -0.00100000f * psi -0.00104481f * w_z;
      // f_z = -0.70710678f * (o_z - o_z_des) -0.40277214f * v_z + 0.43164000f;


      // tau_x = 0.00264575f * (o_y - o_y_des) -0.00666549f * phi + 0.00209651f * v_y -0.00109968f * w_x;
      // tau_y = 0.00100000f * (o_x - o_x_des) -0.01324960f * theta + 0.02649117f * v_x -0.00065878f * w_y + 0.01036500f * alpha;
      // tau_z = -0.00100000f * psi -0.00102029f * w_z;
      // f_z = -0.70710678f * (o_z - o_z_des) -0.40277214f * v_z + 0.43164000f;
